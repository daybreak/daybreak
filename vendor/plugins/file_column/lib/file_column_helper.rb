...
module FileColumnHelper
  ...
  def url_for_file_column(object, method, options=nil)
	...
	def link_to_file_column(object, method, options=nil)
		url = url_for_file_column(object, method, options)
		file_name = url.split('/').last
		link_to(file_name, url)
	end

	def link_to_lightbox_image(object, method, options=nil)
    include_javascript 'lightbox'
    include_stylesheet 'lightbox'   
		url = url_for_file_column(object, method, options)
		link_to(url.split('/').last, url, :rel => 'lightbox')
	end

  # Same as +url_for_file_column+ but allows you to access different versions
